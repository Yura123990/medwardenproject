...
{
    return view("doctors.d_cardiovascular");
}
    ...
    $results = DB::table('symp_cardio_doctor')
        ...
        ->get();
            ...
            foreach ($results as $result) {
                ...
                for ($i = 1; $i <= $symp_q; $i++) {
                    $avgSymptom = "avg_symptom{$i}";
                    $avgSymptomK = "avg_symptom{$i}_k";
        
                    // Розрахунок нового значення за вашою формулою
                    $newData["res_symptom{$i}"] = ($result->$avgSymptom * $result->$avgSymptomK) / 25;
                }
            
                // Створюємо асоціативний масив для використання в updateOrInsert
                $data = array_merge(['illness_id' => $illnessId], $avgSymptoms);
            
                DB::table('symp_cardio_characteristic')->updateOrInsert(
                    ['illness_id' => $illnessId],
                    $data
                );

                DB::table('symp_cardio_res')->updateOrInsert(
                    ['illness_id' => $illnessId],
                    $newData,
                );
            }
